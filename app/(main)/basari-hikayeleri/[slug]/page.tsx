...
// scroll reveal ve time-line animasyon kancaları
import { motion, useReducedMotion } from "framer-motion";
import { ArrowLeft, ArrowRight, Calendar, CheckCircle2, TrendingUp, Users, Target, Shield, Landmark } from "lucide-react";
import Link from "next/link";
import { notFound } from "next/navigation";
import { caseStudies } from "@/lib/case-studies";
import ContactForm from "@/components/ContactForm";

// Dinamik rota parametreleri için statik parametre üretimi
...
export default function CaseStudyDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Öneri başarı hikayeleri (mevcut hikaye hariç 2 tane)
  const relatedStories = caseStudies
    .filter((s) => s.slug !== study.slug)
    .slice(0, 2);

  // Metrik kartları için veri eşleme
  const getMetricIcon = (label: string) => {
    if (label.includes("Tıklama")) return TrendingUp;
    return Shield;
  };
      ...
      <section className="py-24 bg-gradient-to-b from-[#050814] to-[#0a0f25] relative overflow-hidden">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start mb-24">
                  ...
                  <div key={chart.label}>
                    ...
                    <div className="flex gap-4 items-end h-24 w-full border-b border-white/10 pb-2">
                      <div className="flex-1 flex items-end h-full">
                        <div 
                          className="w-full bg-white/10 rounded-t-md transition-all duration-1000 h-0"
                          style={{ height: `${(chart.before / Math.max(chart.before, chart.after)) * 100}%` }}
                          data-testid="before-bar"
                        />
                      </div>
                      <div className="flex-1 flex items-end h-full">
                        <div 
                          className="w-full bg-gradient-to-t from-purple-600 to-pink-500 rounded-t-md transition-all duration-1000 h-0"
                          style={{ height: `${(chart.after / Math.max(chart.before, chart.after)) * 100}%` }}
                          data-testid="after-bar"
                        />
                      </div>
                    </div>
            ...
            {/* Metrik Kartları (Sayaçsız, Saf CSS/Framer Motion Entegre) */}
          ...
          {/* Strateji Anlatısı (Detaylı Bilgi) */}
